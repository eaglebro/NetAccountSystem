    ...
    <form action="" method="">
        ...
        <div class="search_add">
            ...
            <input type="button" value="增加" class="btn_add"
                   onclick="location.href='<%=request.getContextPath()%>/admin/add';"/>
        </div>
        <!--删除和密码重置的操作提示-->
        ...
        <div id="data">
            <table id="datalist">
                ...
                <c:forEach items="${list}" var="adminListVO">
                    <tr>
                        <td><input type="checkbox"/></td>
                        <td>${adminListVO.admin_id}</td>
                        <td>${adminListVO.real_name}</td>
                        <td>${adminListVO.username}</td>
                        <td>${adminListVO.telephone}</td>
                        <td>${adminListVO.email}</td>
                        <td>${adminListVO.create_time}</td>
                        <td>
                            <a class="summary" onmouseover="showDetail(true,this);"
                               onmouseout="showDetail(false,this);">${adminListVO.roleInfo.get(0).name}<c:if test="${adminListVO.roleInfo.size()>1}">...</c:if></a>
                            <!--浮动的详细信息-->
                            <div class="detail_info">
                                <c:forEach items="${adminListVO.roleInfo}" var="roleInfo" varStatus="stat">
                                    ${roleInfo.name}
