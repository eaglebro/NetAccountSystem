...
<div id="main">
    <form action="" method="">
        ...
        <div class="search_add">
            ...
            <input type="button" value="增加" class="btn_add" onclick="location.href='../cost/toAdd.do';"/>
        </div>
        <!--启用操作的操作提示-->
            ...
            <table id="datalist">
                ...
                <c:forEach items="${list}" var="cost">
                    <tr>
                        ...
                        <td>${cost.base_duration==0?"-":cost.base_duration} 小时</td>
                        <td>${cost.base_cost==0?"-":cost.base_cost} 元</td>
                        <td>${cost.unit_cost==0?"-":cost.unit_cost} 元/小时</td>
                        <td>${cost.creatime}</td>
                        <td>${cost.startime}</td>
                        <td>暂停</td>
                        <td>
                            <input type="button" value="启用" class="btn_start" onclick="startFee();"/>
                            <input type="button" value="修改" class="btn_modify" onclick="location.href='<%=request.getContextPath()%>/cost/toModi.do?cost_id=${cost.cost_id}';"/>
                            <input type="button" value="删除" class="btn_delete"
                                   onclick="
                                           var r = window.confirm(' 确定要删除此资费吗？');
                                           if (r) {
                                           location.href='<%=request.getContextPath()%>/cost/delete.do?cost_id=${cost.cost_id}';
                                           }
                                           "/>
                        </td>
                    </tr>
