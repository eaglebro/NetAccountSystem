    ...
    <form action="" method="">
        ...
        <div class="search_add">
            <input type="button" value="增加" class="btn_add"
                   onclick="location.href='<%=request.getContextPath()%>/role/add';"/>
        </div>
        <!--删除的操作提示-->
        ...
        <div id="data">
            <table id="datalist">
                ...
                <c:forEach items="${list}" var="roleVO">
                    <tr>
                        <td>${roleVO.role_id}</td>
                        <td>${roleVO.name}</td>
                            <%--<td>角色管理、管理员管理、资费管理、账务账号、业务账号、账单、报表</td>--%>
                        <td>
                            <c:forEach items="${roleVO.moduleInfoList}" var="moduleInfo" varStatus="stat">
                                ${moduleInfo.name}
                                <c:if test="${!stat.last}">、 </c:if>
                            </c:forEach></td>
                        <td>
                            <input type="button" value="修改" class="btn_modify"
                                   onclick="location.href='<%=request.getContextPath()%>/role/modify?role_id=${roleVO.role_id}';"/>
