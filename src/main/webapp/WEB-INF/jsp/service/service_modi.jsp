...
<div id="main">
    ...
    <form action="<%=request.getContextPath()%>/service/modifyPost" method="post" class="main_form" id="main_form">
        <!--必填项-->
        <div class="text_info clearfix"><span>业务账号ID：</span></div>
        <div class="input_info">
            <input type="text" value="${service.service_id}" name="service_id" readonly class="readonly"/>
        </div>
        ...
        <div class="input_info">
            <select class="width150" name="cost_id">
                <c:forEach items="${costList}" var="cost">
                    <option value="${cost.cost_id}" id="${cost.name}">${cost.name}</option>
                </c:forEach>
            </select>
